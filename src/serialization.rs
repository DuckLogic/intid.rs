    ...
    V: Deserialize<'de>,
{
    ...
    where
        M: MapAccess<'de>,
    {
        let mut result = IdMap::with_capacity_other(access.size_hint().unwrap_or(0));
...
impl<'de, T> Visitor<'de> for IdSetVisitor<T>
where
    T: IntegerId + Deserialize<'de>,
{
    ...
    where
        A: SeqAccess<'de>,
    {
        let mut result = IdSet::with_capacity(seq.size_hint().unwrap_or(0));
