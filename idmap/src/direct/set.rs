...
use intid::{keys::EquivalentIntKey, IntegerId};
...
impl<T: IntegerId> DirectIdSet<T> {
    ...
    #[inline]
    pub fn remove(&mut self, value: impl EquivalentIntKey<T>) -> bool {
        let value = EquivalentIntKey::to_key_index(&value).to_int();
    ...
    #[inline]
    pub fn contains(&self, value: impl EquivalentIntKey<T>) -> bool {
        let value = EquivalentIntKey::to_key_index(&value).to_int();
